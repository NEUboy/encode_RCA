...
static void block_shuffle(unsigned char **pixelbuff,int * order,int length,int img_size[2])
{
    ...
    {
        int read_x=order[i]/(img_size[1]/8)*8;
        int read_y=order[i]%(img_size[1]/8)*8;
        int write_x=i/(img_size[1]/8)*8;
        int write_y=i%(img_size[1]/8)*8;
...
void permutate(image *img,double ** chaos_x,int round)
{
    ...
    block_shuffle(pixelbuff,order,length,img_size);
    img->writePixels(pixelbuff);
}
